...
namespace L2_VR_SteamVR_Basics.Scripts
{
    public class BoxingBagPunchStrengthController : MonoBehaviour
    {
        ...
        private void OnCollisionEnter(Collision other)
        {
            ...
            worldSpaceText.text = $"Punch: {punchStrength:F1}";
            ...
            float volume = Mathf.InverseLerp(0f, 15f, punchStrength);

            if (punchSfx != null)
            {
                AudioSource.PlayClipAtPoint(punchSfx, hitPos, volume);
            }

            // TODO 3.4 : Add a particle FX at the location of the punch.
            if (punchFx != null)
            {
                ParticleSystem fx = Instantiate(punchFx, hitPos, Quaternion.identity);
                fx.Play();
