...
namespace L3_VR_SteamVR_Advanced.Scripts.GravityGun
{
    public class GravityGunController : MonoBehaviour
    {
        ...
        // --- Rotation (bonus) settings ---
        [Header("Rotation (Joystick)")]
        [Tooltip("Name: SteamVR Vector2 action created in SteamVR Input (e.g. GravityGunJoystick)")]
        // NOTE: you must create a Vector2 action named GravityGunJoystick in SteamVR Input
        private SteamVR_Action_Vector2 joystickAction;

        [SerializeField] private float rotationSpeed = 180f; // degrees/sec at full stick
        [SerializeField] private float joystickDeadzone = 0.15f; // ignore tiny stick noise
        ...
        private void OnEnable()
        {
            ...
            SteamVR_Actions._default.TouchTriggerGun.onChange += OnTouchTriggerGun;
            SteamVR_Actions._default.ClickTriggerGun.onChange += OnClickTriggerGun;
            SteamVR_Actions._default.ClickTriggerGun.onChange += OnClickTriggerGun;
            joystickAction = SteamVR_Actions._default.GravityGunJoystick;
        ...
        private void OnTouchTriggerGun(SteamVR_Action_Boolean fromAction, SteamVR_Input_Sources fromSource, bool touchedTriggereState)
        {
            if (touchedTriggereState)
            {
                SnapObject();
            } else
            {
        ...
        private void OnClickTriggerGun(SteamVR_Action_Boolean fromAction, SteamVR_Input_Sources fromSource, bool touchedTriggereState)
        {
            ThrowObject();
        }
        ...
        {
            // Only rotate if there's a snapped object and gun is grabbed.
            if (_selectedObject == null) return;

            Vector2 stick = joystickAction.GetAxis(SteamVR_Input_Sources.Any);

            //if (stick.magnitude < joystickDeadzone) return;

            // Speed scaled by the stick analog magnitude (so half-tilt => half-speed)
            //float scaledSpeed = rotationSpeed * stick.magnitude;
            float scaledSpeed = 90;

            // Compute deltas
            float yawDegrees = stick.x * scaledSpeed * Time.deltaTime;   // left-right
            float pitchDegrees = stick.y * scaledSpeed * Time.deltaTime; // up-down (negated to feel natural)
          

            _selectedObject.Root.transform.Rotate(pitchDegrees, yawDegrees, 0f, Space.Self);
        }
    }
}
